...
export default function RoutesPageClient() {
	...
	const proceedWithBooking = async () => {
		...
		try {
			const createBookingRequest: CreateBookingRequest = {
				...
				flightInstanceIds: bookingForm.flightIds,
				pieces: parseInt(bookingForm.pieces),
				weightKg: parseFloat(bookingForm.weight),
			};
	...
	const handleSearch = async (e: React.FormEvent) => {
		...
		try {
			const response = await searchRoutes(searchData);
			setResults(response);
		} catch (err) {
			setError(
